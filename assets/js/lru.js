...
document.getElementById("lru-submit").addEventListener("click", LRU);
function LRU() {
    let res = document.getElementById("lru-input").value.split(" ");
    let frames = document.getElementById("lru-frames").value;
    let ref = [];
    ...
    createTable("lru-table", frames, ref);
    let obj = new lru(frames);
    for (let i = 0; i < ref.length; i++) {
        obj.refer(ref[i]);
        fillcol("lru-table", i + 1, obj.frames, frames, obj.searchIndex);
    }
    summary('lru-summary', obj.pageFaults, obj.pageFaults + obj.pageHits, frames);
}
