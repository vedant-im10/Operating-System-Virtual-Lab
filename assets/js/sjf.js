...
let s_animate = $("#sjf-animateAll").html();
...
$(document).ready(function () {
    ...
    $("#check").on('change', function () {
        ...
        if (ans == true) {
            check = ans;
            $("#sjf-container").css("display", "none");
            $("#sjf-container_IO").css("display", "grid");
            $("#data_IO").css("display", "grid");
            $("#sjf-data").css("display", "none");
            $("#process").val(1);
            deleteOther();
            lst = 1;
        }
            ...
            check = ans;
            $("#sjf-container_IO").css("display", "none");
            $("#sjf-container").css("display", "grid");
            $("#sjf-data").css("display", "grid");
    ...
    $("#sjf-add_row").click(function () {
        let n = $("#process").val();
        $("#process").val(parseInt(n) + 1);
        if (check == false) {
            $("#sjf-data").append(s);
            $("#sjf-data .cen").eq(lst * 3).text(lst);
        }
        else {
            $("#sjf-data_IO").append(s_IO);
            $("#sjf-data_IO .cen").eq(lst * 4).text(lst);
        }
        lst++;
    });

    $("#sjf-delete_row").click(function () {
        lst--;
        if (lst < 0) {
            lst = 0;
            return;
        ...
        $("#process").val(lst);
        if (check == false) {
            $("#sjf-data").children(".cen").eq(lst * 3 + 2).remove();
            $("#sjf-data").children(".cen").eq(lst * 3 + 1).remove();
            $("#sjf-data").children(".cen").eq(lst * 3).remove();
            $("#sjf-data").children(".ans").eq(lst * 3 + 2).remove();
            $("#sjf-data").children(".ans").eq(lst * 3 + 1).remove();
            $("#sjf-data").children(".ans").eq(lst * 3).remove();
        }
        else {
    ...
    function fun_animation() {
        ...
        while (1) {
            ...
            if (j == -2) {
                ...
                i++;
                $("#sjf-animateAll").append(s_animate);
    ...
    $("#sjf-compute").click(function () {
        ...
        let total_Burst = [];
        if (check == false) {
            let texts = $("#sjf-data .cen").map(function () {
                return $(this).val();
            }).get();
            console.log(texts);
        ...
        if (check == false) {
            for (let i = 0, j = 0; i < 3 * n; i += 3, j++) {
                $("#sjf-data .ans").eq(i).text(Completion[j]);
                $("#sjf-data .ans").eq(i + 1).text(tat[j]);
                $("#sjf-data .ans").eq(i + 2).text(wt[j]);
                avg_tat += tat[j];
                avg_wat += wt[j];
            }
        }
        ...
        $("#sjf-avg_tat").text(Math.round(avg_tat / n * 100) / 100);
        $("#sjf-avg_wat").text(Math.round(avg_wat / n * 100) / 100);
