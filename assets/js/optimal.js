...
document.getElementById("optimal-submit").addEventListener("click", OPTIMAL);
function OPTIMAL() {
    let res = document.getElementById("optimal-input").value.split(" ");
    let frames = document.getElementById("optimal-frames").value;
    let ref = [];
    ...
    createTable("optimal-table", frames, ref);
    let obj = new optimal(frames);
    for (let i = 0; i < ref.length; i++) {
        obj.refer(ref[i], ref.slice(i + 1));
        fillcol("optimal-table", i + 1, obj.frames, frames, obj.searchIndex);
    }
    summary('optimal-summary', obj.pageFaults, obj.pageFaults + obj.pageHits, frames);
}
