...
document.getElementById("lifo-submit").addEventListener("click", LIFO);
function LIFO() {
    let res = document.getElementById("lifo-input").value.split(" ");
    let frames = document.getElementById("lifo-frames").value;
    let ref = [];
    ...
    createTable("lifo-table", frames, ref);
    let obj = new lifo(frames);
    for (let i = 0; i < ref.length; i++) {
        obj.refer(ref[i]);
        fillcol("lifo-table", i + 1, obj.frames, frames, obj.searchIndex);
    }
    summary('lifo-summary', obj.pageFaults, obj.pageFaults + obj.pageHits, frames);
}
