let s = $("#lrtf-data").html();
let s_IO = $("#lrtf-data_IO").html();
let s_animate = $("#lrtf-animateAll").html();
let burst_IO =
...
$(document).ready(function () {
  ...
  $("#check").on("change", function () {
    ...
    if (ans == true) {
      check = ans;
      $("#lrtf-container").css("display", "none");
      $("#lrtf-container_IO").css("display", "grid");
      $("#lrtf-data_IO").css("display", "grid");
      $("#lrtf-data").css("display", "none");
      $("#process").val(1);
      deleteOther();
      lst = 1;
    } else {
      check = ans;
      $("#lrtf-container_IO").css("display", "none");
      $("#lrtf-container").css("display", "grid");
      $("#lrtf-data").css("display", "grid");
      $("#lrtf-data_IO").css("display", "none");
      $("#process").val(1);
      deleteOther();
      lst = 1;
    }
  ...
  function deleteOther() {
    $("#lrtf-data").html(s);
    $("#lrtf-data_IO").html(s_IO);
    $("#lrtf-animateAll").html(s_animate);
    makeHide();
  }
  ...
  $("#lrtf-delete_row").click(function () {
    lst--;
    if (lst < 0) {
      lst = 0;
      return;
    ...
    for (let i = 0; i < lst; i++) {
      ...
      $("#lrtf-data_IO")
        ...
        .change(function () {
          ...
          for (let j = 0; j < t; j++) {
            $("#lrtf-data_IO div").eq(i).append(burst_IO);
          }
        });
    }
  }, 1000);
  ...
  function fun_animation() {
    ...
    while (1) {
      ...
      if (j == -2) {
        ...
        i++;
        $("#lrtf-animateAll").append(s_animate);
  ...
  function select_process_IO(last, arrival_sort) {
    ...
    for (
      ...
      if (max <= total_bt[arrival_sort[i][1]]) {
        if (max == total_bt[arrival_sort[i][1]]) {
    ...
    if (first[0][0] > last) {
      ...
      //     else{
      //         arrival_sort.push([(last +1), first[0][1]]);
  ...
  $("#lrtf-compute").click(function () {
    ...
    if (check == false) {
      ...
      let texts = $("#lrtf-data_IO .cen")
        .map(function () {
          return $(this).val();
        })
        .get();
      let allBT = $("#lrtf-data_IO .cen_IO")
        .map(function () {
          return $(this).val();
        })
        .get();
    ...
    if (check == false) {
      for (let i = 0, j = 0; i < 3 * n; i += 3, j++) {
        $("#lrtf-data .ans").eq(i).text(Completion[j]);
        $("#lrtf-data .ans")
          .eq(i + 1)
          .text(tat[j]);
        $("#lrtf-data .ans")
      ...
      for (let i = 0, j = 0; i < 3 * n; i += 3, j++) {
        $("#lrtf-data_IO .ans").eq(i).text(Completion[j]);
        $("#lrtf-data_IO .ans")
          .eq(i + 1)
          .text(tat[j]);
        $("#lrtf-data_IO .ans")
    ...
    $("#lrtf-avg_tat").text(Math.round((avg_tat / n) * 100) / 100);
    $("#lrtf-avg_wat").text(Math.round((avg_wat / n) * 100) / 100);
