...
function bel(pages, frames) {
    let obj = new belady(frames);
    for (let i = 0; i < pages.length; i++) {
        obj.refer(pages[i]);
    }
    return obj.pageFaults;
}
document.getElementById("belady-submit").addEventListener("click", BELADY);
function BELADY() {
    let res = document.getElementById("belady-input").value.split(" ");
    let ref = [];
    ...
    function bldtable(e) {
        ...
        createTable("belady-table", e.dataPoint.x, ref);
        let obj = new belady(e.dataPoint.x);
        for (let i = 0; i < ref.length; i++) {
            obj.refer(ref[i]);
            fillcol("table", i + 1, obj.frames, e.dataPoint.x, obj.searchIndex);
        }
        summary("belady-summary", e.dataPoint.y, obj.pageHits + obj.pageFaults, e.dataPoint.x)
    }
}
